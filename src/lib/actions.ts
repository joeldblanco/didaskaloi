...
const mergeStudentsSchema = z
  .object({
    firstStudentId: z.string().min(1),
    secondStudentId: z.string().min(1),
    keepStudentId: z.string().min(1),
  })
  .refine((data) => data.firstStudentId !== data.secondStudentId, {
    message: "Debes seleccionar dos estudiantes distintos",
    path: ["secondStudentId"],
  })
  .refine(
    (data) =>
      data.keepStudentId === data.firstStudentId ||
      data.keepStudentId === data.secondStudentId,
    {
      message: "Debes elegir cuál de los dos estudiantes conservar",
      path: ["keepStudentId"],
    },
  );
...
export async function createClass(data: ClassFormValues, projectId?: string) {
  ...
  try {
    ...
    if (existingClass) {
      return {
        success: false,
        error: "Ya existe una clase con este nombre en este proyecto",
      };
    }
...
export async function updateClass(data: ClassFormValues) {
  ...
  try {
    ...
    const { hasAccess, projectId } = await verifyClassAccess(
      user.id,
      validatedData.id,
      Role.EDITOR,
    );
    if (!hasAccess || !projectId) {
      return {
        success: false,
        error: "No tienes permisos para actualizar esta clase",
      };
    }
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === "P2002") {
        return {
          success: false,
          error: "Ya existe otra clase con este nombre",
        };
      }
    }
...
export async function createStudent(data: StudentFormValues) {
  ...
  try {
    ...
    const { hasAccess } = await verifyClassAccess(
      user.id,
    ...
    if (!hasAccess) {
      return {
        success: false,
        error: "No tienes permisos para crear estudiantes en esta clase",
      };
    }
...
export async function updateStudent(data: StudentFormValues) {
  ...
  try {
    ...
    if (!hasAccess) {
      return {
        success: false,
        error: "No tienes permisos para actualizar estudiantes en esta clase",
      };
    }
...
export async function deleteStudent(id: string) {
  try {
    ...
    if (!student) {
      ...
      export async function mergeStudents(data: {
        firstStudentId: string;
        secondStudentId: string;
        keepStudentId: string;
      }) {
        const validatedData = mergeStudentsSchema.parse(data);
        ...
        try {
          ...
          const students = await prisma.student.findMany({
            where: {
              id: {
                in: [
                  validatedData.firstStudentId,
                  validatedData.secondStudentId,
                ],
              },
            },
            select: {
          ...
          if (students.length !== 2) {
            return {
              success: false,
              error: "No se encontraron ambos estudiantes para fusionar",
            };
          }
          ...
          if (firstStudent.classId !== secondStudent.classId) {
            return {
              success: false,
              error: "Solo puedes fusionar estudiantes de la misma clase",
            };
          }
          ...
          if (!hasAccess) {
            return {
              success: false,
              error:
                "No tienes permisos para fusionar estudiantes en esta clase",
            };
          }

          const keptStudentId = validatedData.keepStudentId;
          const removedStudentId =
            keptStudentId === validatedData.firstStudentId
              ? validatedData.secondStudentId
              : validatedData.firstStudentId;
          ...
          await prisma.$transaction(async (tx) => {
            const attendanceRecords = await tx.attendanceRecord.findMany({
              where: {
                studentId: {
                  in: [keptStudentId, removedStudentId],
                },
              },
              orderBy: {
                createdAt: "asc",
            ...
            for (const record of attendanceRecords) {
              ...
              if (record.studentId === keptStudentId) {
                existing.kept = record;
              } else {
                existing.removed = record;
              }

              recordsByAttendance.set(record.attendanceId, existing);
            }
          ...
          return { success: true, keptStudentId };
        } catch (error) {
          ...
          if (error instanceof z.ZodError) {
            return {
              ...
              error:
                ...
                "Datos inválidos para fusionar estudiantes",
            };
          }
    ...
    if (!hasAccess) {
      return {
        success: false,
        error: "No tienes permisos para eliminar estudiantes en esta clase",
      };
    }
...
// Age Range actions
export async function createAgeRange(
  ...
  try {
    ...
    if (!hasEditPermission) {
      return {
        success: false,
        error: "No tienes permisos para crear rangos de edad",
      };
    }
...
export async function updateAgeRange(data: AgeRangeFormValues) {
  ...
  try {
    ...
    if (!hasEditPermission) {
      return {
        success: false,
        error: "No tienes permisos para actualizar este rango de edad",
      };
    }
...
export async function deleteAgeRange(id: string) {
  try {
    ...
    if (!hasEditPermission) {
      return {
        success: false,
        error: "No tienes permisos para eliminar este rango de edad",
      };
    }
...
export async function createAttendance(data: AttendanceFormValues) {
  ...
  try {
    ...
    if (!hasAccess) {
      return {
        success: false,
        error: "No tienes permisos para crear asistencia en esta clase",
      };
    }
...
export async function updateAttendanceRecord(data: AttendanceRecordFormValues) {
  ...
  try {
    ...
    if (!hasAccess) {
      return {
        success: false,
        error: "No tienes permisos para actualizar la asistencia en esta clase",
      };
    }
...
export async function deleteAttendance(id: string) {
  try {
    ...
    if (!hasAccess) {
      return {
        success: false,
        error: "No tienes permisos para eliminar la asistencia en esta clase",
      };
    }
